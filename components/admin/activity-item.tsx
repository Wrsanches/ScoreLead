...
const ACCENTS: Record<ActivityAccent, { icon: string; bg: string; ring: string }> = {
  emerald: { icon: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10", ring: "ring-emerald-500/20" },
  sky:     { icon: "text-sky-600 dark:text-sky-400",     bg: "bg-sky-500/10",     ring: "ring-sky-500/20" },
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        ...
        {time && <p className="text-xs text-zinc-500 dark:text-zinc-600 mt-0.5 tabular-nums">{time}</p>}
      </div>
    </div>
  )
}
