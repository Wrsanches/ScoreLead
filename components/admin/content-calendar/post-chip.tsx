...
export function PostChip({ post, onSelect, compact = true, draggable = true }: PostChipProps) {
  ...
  const bgClass = pillar ? pillar.bgClass : "bg-zinc-200/40 dark:bg-white/[0.07]"
  const ringClass = pillar ? pillar.ringClass : "ring-white/[0.08]"
  ...
  return (
    <div
      ...
      className={`group relative select-none rounded-md border border-zinc-200/80 dark:border-white/[0.08] ${bgClass} ring-1 ${ringClass} hover:border-zinc-300 dark:hover:border-white/[0.22] hover:brightness-110 transition-all duration-150 ${
        draggable ? "cursor-grab active:cursor-grabbing" : "cursor-pointer"
      } ${compact ? "px-2 py-1" : "px-2.5 py-2"}`}
    >
      <div className="flex items-center gap-1.5 min-w-0">
        {post.images && post.images.length > 0 ? (
          <span className="relative shrink-0 w-5 h-5 rounded-sm overflow-hidden border border-zinc-200 dark:border-white/[0.08] block">
            <Image
            ...
            {post.images.length > 1 && (
              <span className="absolute -top-1 -right-1 bg-white dark:bg-black/25 text-zinc-700 dark:text-zinc-300 text-[8px] font-bold leading-none px-1 py-0.5 rounded-sm border border-zinc-300 dark:border-white/[0.14]">
                {post.images.length}
              </span>
            )}
            <span
