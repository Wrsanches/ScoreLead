...
const SCORE_CATEGORIES = [
  { key: "reach", label: "reach", color: "from-emerald-500 to-emerald-400", empty: "bg-emerald-500/[0.06]", text: "text-emerald-600 dark:text-emerald-400" },
  { key: "trust", label: "trust", color: "from-sky-500 to-sky-400", empty: "bg-sky-500/[0.06]", text: "text-sky-600 dark:text-sky-400" },
        ...
        <div className="relative px-6 pt-6 pb-5 border-b border-zinc-200 dark:border-zinc-800/70">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-white tracking-tight line-clamp-2">
                {lead.name || "Unknown"}
              </h2>
              {location && (
                <p className="text-zinc-600 dark:text-zinc-400 text-sm mt-1.5 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-600 shrink-0" />
                  <span className="truncate">{location}</span>
                </p>
              )}
              {lead.googleRating != null && (
                <div className="inline-flex items-center gap-1.5 mt-2 text-xs text-zinc-600 dark:text-zinc-400">
        ...
        <div className="border-t border-zinc-200 dark:border-zinc-800 px-6 py-3 flex items-center justify-between">
          <span className="text-[11px] text-zinc-500 dark:text-zinc-600 capitalize">
            ...
            {lead.firecrawlEnriched && (
              <span className="ml-2 text-zinc-600 dark:text-zinc-400">• enriched</span>
            )}
          </span>
          <Link
...
function ContactRow({
  ...
  return (
    ...
    >
      ...
      {external && (
        <ExternalLink className="w-3 h-3 text-zinc-500 dark:text-zinc-600 group-hover:text-zinc-600 dark:group-hover:text-zinc-400 shrink-0 transition-colors" />
      )}
    </a>
  )
}
